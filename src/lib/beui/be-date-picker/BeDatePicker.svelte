<script>
	...
	import clickOutside from '@/lib/beui/_actions/clickOutside';
	import { FormatTime } from '@/lib/beui/utils/beerui';
...
{#if ranged}
	<div class='be-date be-range' use:clickOutside={{ cb: handleCloseDatePopper }} on:outside={handleCloseDatePopper}>
		...
		>
			...
			<input autocomplete='off' placeholder={startPlaceholder} bind:value={displayValue[0]} disabled={disabled}
						 readonly={readonly} on:input={handleStartInput} on:change={handleStartChange} on:focus={handleFocus}
						 class='be-range-input'>
			<span class='be-range-separator'>{separator}</span>
			<input autocomplete='off' placeholder={endPlaceholder} bind:value={displayValue[1]} disabled={disabled}
						 readonly={readonly} on:input={handleStartInput} on:change={handleStartChange} on:focus={handleFocus}
						 class='be-range-input'>
			<div
...
{:else}
	<div class='be-date' use:clickOutside={{ cb: handleCloseDatePopper }} on:outside={handleCloseDatePopper}
		...
		<Dates {valueFormat} {direction} {selectableRange} {disabledDate} {value} {selectMode} {format}
					 bind:visible={visible} on:pick={confirmPick} />
	</div>
{/if}
