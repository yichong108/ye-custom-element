<script>
  ...
  export let bottom = {status: 'init', value: 0};
...
<div class='be-cascader-panel' bind:this={cascaderRect} bind:clientWidth={cascaderWidth} class:visible={visible}>
  {#if menus && menus.length > 0}
    {#each menus as menu, index}
      <CascaderMenu {expandTrigger} {selectValue} {config} {menu} bind:curLoadingId={curLoadingId} {checkStrictly}
                    {lazy} value={ value[index] || selectValue } {store}/>
    {/each}
  {:else}
