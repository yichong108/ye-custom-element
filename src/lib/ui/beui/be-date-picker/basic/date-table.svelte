<script>
  ...
  function initDate(date) {
    ...
    dateCountOfLastMonth = getDayCountOfMonth(
      year,
      month === 0 ? 11 : month - 1
    );
    // 每月第一天是星期几
  ...
  function getMonthArray() {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        ...
        cell.inRange =
          time >= getDateTimestamp(minDate) &&
          time <= getDateTimestamp(maxDate);
        ...
        //  禁用日期
        cell.disabled =
          typeof disabledDate === "function" && disabledDate(curDate);
        row.push(cell);
      }
    }
  }
  ...
  function selectDay(e, cell, index) {
    ...
    if (selectMode === "range") {
      ...
      const dateTime = value
        ? modifyDate(date, year, dateMonth, cell.text)
        : newDate;
