<script>
  ...
  import BeButton from "@/lib/ui/beui/be-button/BeButton.svelte";
  import BeInput from "@/lib/ui/beui/be-input/BeInput.svelte";
  ...
  $: leftLabel =
    ...
    "年" +
    " " +
    String(leftDate.getMonth() + 1).padStart(2, "0") +
    "月";
  $: rightLabel =
    rightDate.getFullYear() +
    "年" +
    " " +
    String(rightDate.getMonth() + 1).padStart(2, "0") +
    "月";
  ...
  let popperArrow;
  // 时分秒格式化结构
  const times = new FormatTime(
    format.substring(format.indexOf("HH"), format.length)
  );
  $: if (visible) {
    // 处理右边边界问题
...
{#if visible}
  <div
    class="be-picker-panel be-date-picker be-range-daterange-picker be-datetime-range-{direction} be-popper"
    bind:this={rangeRect}
  ...
  >
    ...
    >
      <div class="be-range-daterange-picker__header">
        <span
          role="button"
          tabindex="-1"
          class="be-picker-panel__icon-btn be-date-picker__prev-btn"
        ...
        <span
          role="button"
          tabindex="-1"
          class="be-picker-panel__icon-btn be-date-picker__d-prev-btn"
          on:click={handlePrevYear}
          on:keydown
        ></span>
        <span role="button" tabindex="-1" class="be-date-picker__header-label"
          >{leftLabel}</span
        >
      </div>
      <DateTable
    ...
    >
      <div class="be-range-daterange-picker__header">
        ...
        <span
          role="button"
          tabindex="-1"
          class="be-picker-panel__icon-btn be-date-picker__next-btn"
        ...
        <span
          role="button"
          tabindex="-1"
          class="be-picker-panel__icon-btn be-date-picker__d-next-btn"
    ...
    <div class="be-picker-panel__footer be-picker-panel__footer-datetime">
      <div class="be-date-picker__range-time-header">
        <div
          class="be-date-picker__time-header"
          use:clickOutside={{ cb: handleCloseTimePopper }}
          on:outside={handleCloseTimePopper}
        >
          <BeInput
            validateEvent={false}
            placeholder="选择时间"
          ...
          <Time
            ...
            {format}
            visible={timeVisible}
            on:pick={confirmTimePick}
          />
        </div>
      </div>
      <div
        class="be-date-picker__range-time-header be-picker-panel__footer-datetime"
      >
        <div
          class="be-date-picker__time-header"
          use:clickOutside={{ cb: handleCloseRightTimePopper }}
          on:outside={handleCloseRightTimePopper}
        >
          <BeInput
            validateEvent={false}
            placeholder="选择时间"
            readonly
            value={timeRightValue}
            on:focus={handleShowRightTimePopper}
          />
          <Time
            date={maxDate}
            direction="top"
            {format}
            visible={timeRightVisible}
            on:pick={confirmRightTimePick}
          />
        </div>
        <BeButton
          type="default"
