...
{#if visible}
  <div
    class="be-picker-panel be-date-picker be-popper be-{selectMode}-{direction}"
      ...
      <div class="be-picker-panel__body">
        <div class="be-date-picker__header">
          ...
          <span
            ...
            on:keydown={handlePrevYear}
          ></span>
          {#if currentView !== "year"}
            <span
              role="button"
              tabindex="-1"
              class="be-date-picker__header-label"
              on:click={() => (currentView = "year")}
              on:keydown={() => (currentView = "year")}>{year} 年</span
            >
          {/if}
          {#if currentView === "year"}
          ...
          {#if currentView === "date"}
            <span
              ...
              on:keydown={() => (currentView = "month")}>{month} 月</span
            >
          {/if}
          {#if currentView === "date"}
            <span
    ...
    {#if selectMode === "datetime"}
      <div class="be-picker-panel__footer be-picker-panel__footer-datetime">
        ...
        <BeButton
          type="default"
          size="mini"
          on:click={confirmDateTimePick}
          on:keydown={confirmDateTimePick}>确认</BeButton
        >
      </div>
    {/if}
    {#if direction == "bottom"}
