<script>
  ...
  const isSelectHandle = (item) => {
    isSelect =
      store.multiple && Array.isArray(item)
        ? item.includes(value)
...
<li
  ...
  class:hover
>
  <div role="button" tabindex="-1" on:click={handleClick} on:keydown>
    {#if $$slots.default}
      <slot prop={value} />
    {:else}
      {label}
      ...
      <div class="be-icon-inner" class:is-block={isSelect}>
        ...
        {:else}
          <div class="be-icon" style="width: 16px; height: 16px;">
            ...
            >
              <path
                d="M6.43348 9.92151L12.6561 3.69885L13.5754 4.61809L6.43348 11.76L1.97412 7.30063L2.89336 6.38139L6.43348 9.92151Z"
                fill="currentColor"
                fill-opacity="0.9"
              ></path>
            </svg>
