<script>
  ...
  $: leftLabel =
    Math.floor(leftDate.getFullYear() / 10) * 10 +
    " - " +
    (Math.floor(leftDate.getFullYear() / 10) * 10 + 9);
  $: rightLabel =
    Math.floor(rightDate.getFullYear() / 10) * 10 +
    " - " +
    (Math.floor(rightDate.getFullYear() / 10) * 10 + 9);
...
{#if visible}
  <div
    class="be-picker-panel be-date-picker be-range-daterange-picker be-popper be-month-range-{direction}"
    bind:this={rangeRect}
  >
    <div
